...
async function main(): Promise<void> {
  ...
  const dataset: LeaderboardDataset = {
    ...
    notices: [
      ...
      "重要度は PR 作成 10%、実装最大 55%、レビュー 20%、Issue と調査 15%へ配分します。",
      "PR は選択期間の末日時点の状態で判定し、実装枠をマージ済みは 1 倍、オープンは 0.5 倍、クローズ済みは 0.25 倍にします。",
      "PR 作成点は重要度の 10%を集計期間内に作成された PR の人間の作者へ一度配分し、共同作者へは分配しません。作成日が期間外または作者が Bot の場合は配点対象外です。",
      "独立した品質確認がない未マージ PR は、期間内作成で人間の作者がいれば PR 作成点を配分し、実装枠と関連 Issue 枠は配分しません。",
